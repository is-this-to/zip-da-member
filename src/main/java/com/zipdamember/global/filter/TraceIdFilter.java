package com.zipdamember.global.filter;

import com.zipdamember.global.context.TraceIdContext;
import com.zipdamember.global.context.constant.InternalHeaderName;
...
public class TraceIdFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request, @NonNull HttpServletResponse response, @NonNull FilterChain filterChain) throws ServletException, IOException {
        String traceId = resolveTraceId(request.getHeader(InternalHeaderName.X_TRACE_ID));

        TraceIdContext.set(traceId);

        request.setAttribute(TraceIdContext.REQUEST_ATTRIBUTE, traceId);

        response.setHeader(InternalHeaderName.X_TRACE_ID, traceId);
